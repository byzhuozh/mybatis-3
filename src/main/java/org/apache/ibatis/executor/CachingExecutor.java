...
public class CachingExecutor implements Executor {
     ...
     *
     * 事务的缓存管理器。因为二级缓存是支持跨 Session 进行共享，此处需要考虑事务，那么，必然需要做到事务提交时，
    ...
    public <E> List<E> query(MappedStatement ms, Object parameterObject, RowBounds rowBounds, ResultHandler resultHandler, CacheKey key, BoundSql boundSql)
            throws SQLException {
        //调用 MappedStatement#getCache() 方法，获得 Cache 对象，即当前 MappedStatement 对象的二级缓存
    ...
    private void flushCacheIfRequired(MappedStatement ms) {
        ...
        if (cache != null && ms.isFlushCacheRequired()) {// 是否需要清空缓存
            /**
             * 清空缓存。注意，此时清空的仅仅，当前事务中查询数据产生的缓存。而真正的清空，在事务的提交时。
