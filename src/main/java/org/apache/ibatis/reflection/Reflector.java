 ...
 *
 * 反射器  Reflector 会缓存反射操作需要的类的信息，例如：构造方法、属性名、setting / getting 方法等等
 */
public class Reflector {

  /**
   * 对应的类，每个 Reflector 对应一个类。
   */
  ...
  private void addGetMethods(Class<?> cls) {
    ...
    // <4> 解决 getting 冲突方法     最终，一个属性，只保留一个对应的方法
  ...
  private void resolveGetterConflicts(Map<String, List<Method>> conflictingGetters) {
    ...
    for (Entry<String, List<Method>> entry : conflictingGetters.entrySet()) {
      ...
      for (Method candidate : entry.getValue()) {
        ...
        // 不符合选择子类
        } else if (candidateType.isAssignableFrom(winnerType)) {// 判断 candidateType 是否是 winnerType 的父类
          // OK getter type is descendant
          // <1.1> 符合选择子类。因为子类可以修改放大返回值。例如，父类的一个方法的返回值为 List ，子类对该方法的返回值可以覆写为 ArrayList
        } else if (winnerType.isAssignableFrom(candidateType)) {  // 判断 winnerType 是否是 candidateType 的父类
          winner = candidate;
        } else {
  ...
  private Method pickBetterSetter(Method setter1, Method setter2, String property) {
    ...
    if (paramType1.isAssignableFrom(paramType2)) {  // paramType1 是 paramType2 的父类
      return setter2;
  ...
  private void addSetField(Field field) {
    ...
    if (isValidPropertyName(field.getName())) {
      // 添加到 setMethods 中
      setMethods.put(field.getName(), new SetFieldInvoker(field));  // SetFieldInvoker
      // 添加到 setTypes 中
  ...
  private void addGetField(Field field) {
    ...
    if (isValidPropertyName(field.getName())) {
      // 添加到 getMethods 中
      getMethods.put(field.getName(), new GetFieldInvoker(field));   // GetFieldInvoker
      // 添加到 getMethods 中
      Type fieldType = TypeParameterResolver.resolveFieldType(field, type);
